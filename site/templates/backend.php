...
function processResults($results, $mypages) {

    // $categoryParent = $mypages->get($results[0]->id)->parent->parent->get("template=categories");
    // $questionairePage = $mypages->get($results[0]->id)->parent;
    $questionairePage = $mypages->get(1047)->parent;
    // var_dump($questionairePage);
    ...
    foreach($results as $result) {
        ...
        $questionPage = $mypages->get($result->id);
    ...
    foreach($resultsArray['results'] as &$cat) {
        ...
        if ($mypages->get($cat['id'])->scoring->count) {
            $explanation = $mypages->get($cat['id'])->scoring->get('score_value='.$roundedAverage)->score_description;
...
$processedResults = processResults($results, $pages);
